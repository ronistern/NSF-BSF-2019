...
% Planning and diagnosis are important
...
The reliance on an available world model, however, is a known limitation of model-based reasoning. Data-driven methods have been proposed as an alternative to model-based reasoning for performing many AI tasks. %, including planning and diagnosis. 
...
On the other hand, data-driven methods overcome an inherent obstacle in model-based reasoning -- the need for an accurate enough model of the environment. Moreover, Valiant argues that  data-driven mechanisms also compensate for the kind of errors that are introduced as a result of  reasoning with a model that is an approximate representation of the world~\cite{valiant2000neuroidal,valiant2000robustLogics}. %However, unless similar situations were observed in the past, data-driven methods do not provide correctness guarantees (for the generated plan or  for the found diagnosis) while model-based reasoning provides a principled,  well-studied approach to reason about every situation in which the model holds. 

%Data-driven methods overcome an inherent obstacle in model-based reasoning -- the need for an accurate enough model of the environment. Moreover, Valiant argues that  data-driven mechanisms also compensate for the kind of errors that are introduced as a result of  reasoning with a model that is an approximate representation of the world~\cite{valiant2000neuroidal,valiant2000robustLogics}. However, unless similar situations were observed in the past, data-driven methods do not provide correctness guarantees (for the generated plan or  for the found diagnosis) while model-based reasoning provides a principled,  well-studied approach to reason about every situation in which the model holds. 
...
%We propose to {\bf develop a framework for algorithms that use both available observations and world models.} The data-and-model driven approach will be developed for two main settings.
...
%We outline below the challenges involved in doing so and how we aim to address them. 
...
%\paragraph{Objective \#2. Data-driven probably-suboptimal model-based planning.}
...
% We only did preliminary stuff. What more on safe planning: lifted representation
...
%What more on safe planning: lifted representation: ordinal and numeric
